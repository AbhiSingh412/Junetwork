...
<div style='position:relative;float:left;margin-left:2%;width:auto;'>".$myfriendname."&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;<B>".$sub_rows2." UNREAD MESSAGES</b><br><form action='' method='POST'><input type='hidden' name='chatnamedata' value='".$myfriendname."'><input type='hidden' name='chatdata' value='".$myfriend."'><input type='submit' value='Chat' /></form></div>
  ...
  $queery = "SELECT * FROM messages WHERE (friend1='$mobile' && friend2='$reciepent') OR (friend1='$reciepent' && friend2='$mobile')";
  ...
  echo "<div id='chatcontainer' style='margin-top:15px;width:97.5678%;left:1%;height:390px;overflow:auto;position:relative;'>";
  if($roows) {
  for($i=0;$i<$roows;$i++)
  {
     $isread = "NOT READ";
     if(mysql_result($reesult,$i,'friend1') == $mobile) {
       if(mysql_result($reesult,$i,'isread'))
         $isread = "READ";
       echo "<div style='background:lime;margin-left:40%;border-bottom-left-radius:12px;padding:9px;width:30%;'>".mysql_result($reesult,$i,'message').$isread."</div><br>";
     }
     else
      echo "<div style='background:cyan;border-bottom-left-radius:6px;padding:9px;margin-left:0%;width:30%;'>".mysql_result($reesult,$i,'message')."</div><br>";
  ...
  infintepart:
  echo "<script>document.getElementById('chatcontainer').innerHTML += '<!-- something is pending -->";

  $loaderquery = "SELECT * FROM messages WHERE (friend1='$reciepent' && friend2='$mobile') AND isread=0";
  $loaderres = mysql_query($loaderquery);
  if (!$loaderres) die ("Database access failed: " . mysql_error());
  $loaderows = mysql_num_rows($loaderres);

  $updloaderquery = "UPDATE messages set isread=1 WHERE (friend2='$mobile' && friend1='$reciepent')";
  $updloaderres = mysql_query($updloaderquery);
  if (!$updloaderres) die ("Database access failed: " . mysql_error());
  ...
  echo "<!-- pending part complete -->';</script>";
  // echo "<script>var chatcontainer = document.getElementById('chatcontainer');chatcontainer.scrollTop = chatcontainer.scrollHeight;</script>";
  */
